...
(define* (meson-targets #:optional (m (%meson)))
  (set->list (.targets m)))
...
(define*-public (meson-name #:optional (m (%meson)))
  (.name m))
(define*-public (meson-languages #:optional (m (%meson)))
  (set->list (.languages m)))
(define*-public (meson-variables #:optional (m (%meson)))
  (module-map (lambda (x n)
                (list x n))
              (.variables m)))
...
(define* (meson-external-program #:optional (m (%meson)))
